<template lang="html">
  <div :val="value_">
    ...
    <div>
      <el-radio v-model="type" @change="type = '4'" label="4" size="mini" border>指定</el-radio>
      <el-checkbox-group v-model="appoint">
        <div v-for="i in 4" :key="i" style="margin-left: 10px;  line-height: 25px;">
          <template v-for="j in 10">
            <el-checkbox v-if="parseInt((i - 1) + '' + (j - 1)) < 32 && !(i === 1 && j === 1)" :key="j" :label="parseInt((i - 1) + '' + (j - 1))+''">{{(i - 1) + '' + (j - 1)}}</el-checkbox>    
          </template>
        </div>
      </el-checkbox-group>
    </div>
    ...
    value_ () {
      ...
      switch (this.type) {
        ...
        case '3': // 循环
          ...
          break
        case '4': // 指定
            let appoint=this.appoint.join(',')==''?'*':this.appoint.join(',')
            result.push(appoint)
          break
        case '6': // 最后
