<template>
    ...
    >
        <el-empty v-if="!optId" height="1000" description="暂时没有配置相关的库表查询"></el-empty>
        <audaque-table-list-page v-else-if="optId">
            ...
            <template slot="search">
                <audaque-table-search
                    ...
                    v-show="isSearch"
                    @search="REPORT_DISPLAY_LIST_PARAMS(0)"
                    @reset="REPORT_DISPLAY_LIST_PARAMS(1)"
                >
            ...
            <template>
                <el-table
                    ...
                    stripe
                    :data="reportDisplayList.rows"
                    v-loading="reportDisplyTableLoading"
                    element-loading-text="加载中"
                    element-loading-spinner="el-icon-loading"
                    element-loading-background="rgba(255, 255, 255, 0.6)"
                    ref="aaa"
                ...
                <el-dialog :title="drillTitle" :visible.sync="dialogDrillVisible">
                    <el-table :data="reportDrillList.rows" tooltip-effect="dark" style="width: 100%" height="6rem">
                        <el-table-column
                            :label="item.displayColName ? item.displayColName : item.columnComment"
                            align="center"
                            v-for="(item, index) in columnList"
                    ...
                    <div slot="footer" class="dialog-footer">
                        <el-button type="primary" @click="dialogDrillVisible = false">关 闭</el-button>
                    </div>
                </el-dialog>
            </template>
...
export default {
    ...
    data() {
        return {
            ...
            isSearch: false,
            visible: false,
            reportDisplyTableLoading: false,
            loading: false,
            nearlyTimes: 5,
            total: 1,
            param: {
            ...
            dialogDrillVisible: false,
            loadingDrill: false,
            loadingDisplay: false,
        };
    },
    computed: {
        ...mapState('webkit', ['reportDisplayList', 'reportDrillList', 'reportConfigMenuList']),
    },

    methods: {
        ...mapActions(['_AJAX']),
        ...mapActions('webkit', [
            'SET_REPORT_DRILL_LIST',
            'QUERY_REPORT_PROPERTY_ALL',
            'VALIDATE_REPORT_EXPORT_EXCELHIS',
            'EXPORT_REPORT_DATA_HIS',
            'QUERY_REPORT_CONFIG_DETAIL',
            'QUERY_REPORT_DISPLAY_LIST',
            'EXPORT_REPORT_DATA',
            'SET_REPORT_DISPLAY_LIST',
            'QUERY_REPORT_DRILL_LIST',
        ]),
        initData() {
            ...
            this.reportDisplyTableLoading = true;
            this.QUERY_REPORT_DISPLAY_LIST(params)
                .then(() => {
                    this.reportDisplyTableLoading = false;
                })
                .catch(() => {
                    this.reportDisplyTableLoading = false;
                });
        ...
        initColumn() {
            let configId = this.optId;
            let params = { configId: configId, type: 1 };
            return this.QUERY_REPORT_PROPERTY_ALL(params).then((res) => {
                this.columnList = res;
                if (res.length > 0) {
                    this.isShowSeq = true;
                }

                let para = { configId: configId, type: 3 };
                this.QUERY_REPORT_PROPERTY_ALL(para).then((res) => {
        ...
        initQuery() {
            let configId = this.optId;
            let params = { configId: configId, type: 2 };
            return this.QUERY_REPORT_PROPERTY_ALL(params).then((res) => {
                (res || []).forEach((item) => {
                    item.value = '';
        ...
        exportTemplate(api, propertyId) {
            this.publics.exportFile(this, api, { confId: this.optId, propertyId: propertyId, dataSource: 'report' });
        },
        exportFun() {
            this.loading = true;
            let configId = this.optId;
            ...
            this.EXPORT_REPORT_DATA(params)
                .then((res) => {
        ...
        exportFunHis() {
            ...
            this.$store.commit('SHOW_LOADING', true);
            this.VALIDATE_REPORT_EXPORT_EXCELHIS(params)
                .then((res) => {
                    this.EXPORT_REPORT_DATA_HIS(params)
                        .then((res) => {
        ...
        openDrill(row) {
            this.SET_REPORT_DRILL_LIST({}).then((res) => {
                this.QUERY_REPORT_CONFIG_DETAIL({ id: this.optId }).then((res) => {
                    let queryParams = {};
                    (this.queryList || []).forEach((item) => {
                        if (item.value) {
                            queryParams[item.columnName] = item.value;
                    ...
                    this.drillTitle = res.drillTitle;
                    let drillColumn = res.drillColumn;
                    let drillValue = row[drillColumn];
                    if (drillValue == null) {
                        drillValue = -999999999;
                    }
                    let params = {
                        configId: this.optId,
                        params: queryParams,
                    ...
                    this.QUERY_REPORT_DRILL_LIST(params)
                        .then(() => {
        ...
        REPORT_DISPLAY_LIST_PARAMS(num) {
            this.param.page = 1;
            if (num > 0) {
                (this.queryList || []).forEach((item) => {
                    item.value = '';
