        ...
        <template>
            ...
            >
                <el-row>
                    ...
                    <el-col :span="12">
                        <el-form-item label="报表类别" prop="categoryId">
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="配置方式" prop="mode">
                            <el-radio v-model="form.mode" label="1" :disabled="optType === 'edit'">手动配置</el-radio>
                            <el-radio v-model="form.mode" label="2" :disabled="optType === 'edit'">SQL语句</el-radio>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                ...
                <el-row>
                    <el-col v-if="form.mode === '1'" :span="12">
                        <el-form-item label="库表" prop="tableName">
                            <el-select
                            ...
                            >
                                <el-option-group
                                    v-for="group in reportTableListTemp"
                                    :key="group.label"
                                    :label="group.label"
                                >
                    ...
                    <el-col v-if="form.mode === '1'" :span="12">
                        <el-form-item label="展示记录数" prop="limitNum">
                            <el-input v-model.trim="form.limitNum" maxlength="10" show-word-limit></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-row v-if="form.mode === '1'">
                    ...
                    <el-col :span="12">
                        <el-form-item label="下钻关联字段">
                            ...
                            >
                                <el-option
                                    v-for="item in this.reportColumnList"
                                    :key="item.columnName"
...
export default {
    data() {
        ...
        return {
            ...
            rules: {
                reportName: [{ required: true, message: '报表名称不能为空' }],
    ...
    watch: {},
    computed: {
        ...mapState('webkit', ['reportConfigDetail', 'reportCategoryAll', 'reportColumnList']),
    },
    created() {
        this.$store.commit('webkit/REPORT_TABLE_LIST', []);
        this.$store.commit('webkit/REPORT_COLUMN_LIST', []);
    ...
    methods: {
        ...mapActions('webkit', [
            ...
            'QUERY_REPORT_CONFIG_DETAIL',
            'QUERY_REPORT_CATEGORY_All',
            'QUERY_REPORT_TABLE_LIST',
            'QUERY_REPORT_COLUMN_LIST',
        ]),
        async queryDetail() {
            let params = { id: this.optId };
            await this.QUERY_REPORT_CONFIG_DETAIL(params).then((res) => {
                if (this.optType !== 'add') {
                    this.form.reportName = this.reportConfigDetail.reportName;
                    this.form.categoryId = this.reportConfigDetail.categoryId;
                    this.form.tableName = this.reportConfigDetail.tableName;
                    this.form.mode = this.reportConfigDetail.mode;
                    this.form.sortNum = this.reportConfigDetail.sortNum;
                    this.form.limitNum = this.reportConfigDetail.limitNum;
                    this.form.drillColumn = this.reportConfigDetail.drillColumn;
                    this.form.drillTitle = this.reportConfigDetail.drillTitle;
                    this.form.displayColumnList = this.reportConfigDetail.displayColumnList;
                    this.form.queryColumnList = this.reportConfigDetail.queryColumnList;
                    this.form.detailColumnList = this.reportConfigDetail.detailColumnList;
                    this.form.mode = this.reportConfigDetail.mode;
                    this.form.sqlStatement = this.reportConfigDetail.sqlStatement;
                    this.form.createUserName = this.reportConfigDetail.createUserName;
                    this.form.createTime = this.reportConfigDetail.createTime;
                    this.queryReportColumnList(this.form.tableName);
                } else {
                    this.form.limitNum = this.reportConfigDetail.limitNum;
                }
            });
        ...
        async queryReportTableList() {
            let tableParams = {
                tableType: 'BASE TABLE',
            };
            let tableList = await this.QUERY_REPORT_TABLE_LIST(tableParams);
            let viewParams = {
                tableType: 'VIEW',
            };
            let viewList = await this.QUERY_REPORT_TABLE_LIST(viewParams);
            this.reportTableListTemp = [
                {
                    label: '表',
                    options: tableList,
                },
        ...
        queryReportColumnList(val) {
            let params = { table: val };
            this.QUERY_REPORT_COLUMN_LIST(params);
        },
        checkTable(val) {
            this.form.displayColumnList = [];
            this.form.detailColumnList = [];
            this.queryReportColumnList(val);
        },
        toggleSelection() {
            if (this.tableDialogType === 'where') {
                ...
                this.$refs.multipleTable.selection.forEach((item) => {
                    ...
                    obj.columnClicked = 0;
                    obj.columnDrilled = 0;
                    this.form.detailColumnList.push(Object.assign({}, obj));
                });
            }
        ...
        openTableDialog(type) {
            ...
            this.$nextTick(() => {
                ...
                if (type === 'display') {
                    (this.reportColumnList || []).forEach((reportColumn, index) => {
                        (this.form.displayColumnList || []).forEach((displayColumn) => {
                            if (reportColumn.columnName === displayColumn.columnName) {
                                this.$refs.multipleTable.toggleRowSelection(this.reportColumnList[index], true);
                            }
                        });
                    });
                } else if (type === 'where') {
                    (this.reportColumnList || []).forEach((reportColumn, index) => {
                        (this.form.queryColumnList || []).forEach((queryColumn) => {
                            if (reportColumn.columnName === queryColumn.columnName) {
                                this.$refs.multipleTable.toggleRowSelection(this.reportColumnList[index], true);
                            }
                        });
                    });
            ...
            this.$refs.ruleform.validate((valid) => {
                if (valid) {
                    ...
                    if (this.optType === 'edit') {
                        data.id = this.optId;
                        this.UPDATE_REPORT_CONFIG(data)
                            .then((res) => {
