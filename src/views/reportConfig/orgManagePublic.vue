    ...
    <div style="height: 100%">
        <audaque-resize-split-page v-loading="loading">
            ...
            <template slot="rightPane">
                    ...
                    <el-table-column width="120">
                        <template slot-scope="scope">
                            <el-button
                                type="primary"
                                icon="el-icon-plus"
                                @click="selectAdd(scope.row)"
                                size="mini"
                                v-if="!authRows.map((item) => item.userId).includes(scope.row.userId)"
                                circle
                            ></el-button>
                            <el-button
                                type="danger"
                ...
                ></adq-pagination>
                <div class="already_select_accounts" v-if="authRows.length > 0">
                    <span>已授权账户：</span><span>{{ authRows.map((item) => item.account).join('，') }}</span>
                </div>
            </template>
...
export default {
    ...
    methods: {
         ...
         * */
        selectAdd(row) {
            this.$emit('getRows', true, row);
        },
        selectDelete(row) {
            this.$emit('getRows', false, row);
        },
        ...
        handleSelectionChange(val) {
            // 如果以选中
            if (!this.totalSelection.find((item) => val.map((valItem) => valItem.id).includes(item.id))) {
                this.totalSelection.push(val);
            }
        ...
        loadNode(node, resolve) {
            if (node.level === 0) {
                this.treeNode = node;
                this._AJAX({
                    url: this.pageType === 'system' ? 'apiQueryConf.findRootDept' : 'apiQueryConf.findMyDept',
                    method: 'GET',
                    data: {},
                }).then((res) => {
            ...
            this._AJAX({
                url:
                    (this.pageType === 'system'
                        ? API['apiQueryConf.findChildrenDept']
                        : API['apiQueryConf.findMyChildren']) + '?deptId=1',
                method: 'GET',
                params: {
                    deptId: 1,
                },
                        ...
                        this.$refs.ruleForm.validate((valid) => {
                            if (valid) {
                                ...
                                _this
                                    ._AJAX({
                                        url:
                                            _this.pageType === 'system'
                                                ? 'apiQueryConf.setDeptManager'
                                                : 'apiQueryConf.setMyDeptManager',
                                        method: 'POST',
                                        data: {
        ...
        getTreeDataInfo(id) {
            this._AJAX({
                url: this.pageType === 'system' ? 'apiQueryConf.findDepartment' : 'apiQueryConf.findMyDepartment',
                method: 'GET',
                data: {
                    deptId: id,
                },
        ...
        getDeptRole(id, row, type) {
            ...
            if (this.pageType === 'system') {
                ...
                if (row.level === 1) {
                    ...
                    url = type === 'edit' ? 'role.myRoleList' : 'apiQueryConf.getDeptRole';
                } else {
                    url = 'apiQueryConf.getDeptRole';
                }
            } else {
                //组织
                if (row.level === 1) {
                    ...
                    url = type === 'edit' ? 'role.myRoleList' : 'apiQueryConf.getMyDeptRole';
                } else {
                    url = 'apiQueryConf.getMyDeptRole';
                }
            }
            this._AJAX({
                url: url,
        ...
        getDeptTag(id) {
            this._AJAX({
                url: this.pageType === 'system' ? 'apiQueryConf.getDeptTag' : 'apiQueryConf.getMyDeptTag',
                method: 'GET',
                data: {
                    deptId: id,
                },
        ...
        treeDeleteDept() {
            ...
            let _this = this;
            this._AJAX({
                url: this.pageType === 'system' ? 'apiQueryConf.deleteDepartment' : 'apiQueryConf.deleteMyDepartment',
                method: 'POST',
                data: {
                    adq: row.id,
                },
                ...
                if (valid) {
                    ...
                    if (this.treeDialogType === 'add') {
                        ...
                        this._AJAX({
                            url:
                                this.pageType === 'system'
                                    ? 'apiQueryConf.addDepartment'
                                    : 'apiQueryConf.addMyDepartment',
                            method: 'POST',
                            data: params,
                        }).then((res) => {
                            if (res.code == 0) {
        ...
        modifyDept(params, row) {
            let _this = this;
            this._AJAX({
                url: this.pageType === 'system' ? 'apiQueryConf.modifyDepartment' : 'apiQueryConf.modifyMyDepartment',
                method: 'POST',
                data: params,
            }).then((res) => {
                if (res.code == 0) {
        ...
        getUserList() {
            ...
            if (this.clickedRow.id) {
                ...
                this._AJAX({
                    url:
                        this.pageType === 'system'
                            ? 'apiQueryConf.findDeptUserList'
                            : 'apiQueryConf.findMyDeptUserList',
                    method: 'GET',
                    data: params,
                })
                    .then((res) => {
        ...
        delUser(row) {
            ...
            _this
                ._AJAX({
                    url: _this.pageType === 'system' ? 'apiQueryConf.deleteDeptUser' : 'apiQueryConf.deleteMyDeptUser',
                    method: 'POST',
                    data: {
                        deptId: row.deptId,
                        userIds: [row.userId],
        ...
        uploadFile(event) {
            ...
            this.loading = true;
            this._AJAX({
                url: 'apiQueryConf.orgListImport',
                data: this.formData,
                method: 'POST',
            })
                .then((res) => {
        ...
        uploadFile_1(event) {
            ...
            this.loading = true;
            this._AJAX({
                url: 'apiQueryConf.orgUserListImport',
                data: this.formData,
                method: 'POST',
            })
                .then((res) => {
